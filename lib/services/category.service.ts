...
export async function getCategoryData(): Promise<CategoryData[]> {
  try {
    ...
    // 获取所有页面数据
    const allPages = await notionService.getAllData()

    // 转换数据格式
    const pages: NotionCategoryPage[] = allPages.map((page: any) => ({
      id: page.id,
      name: page.properties?.name?.title?.[0]?.plain_text || '未命名',
      desc: page.properties?.desc?.rich_text?.[0]?.plain_text || '',
      sort: page.properties?.sort?.number || 0,
      '上级 项目': page.properties?.['上级 项目']?.relation || null,
      '子级 项目': page.properties?.['子级 项目']?.relation || null,
    }))
